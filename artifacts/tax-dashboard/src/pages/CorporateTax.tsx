...
import {
  useListCorporateTax, useCreateCorporateTaxRecord, useListClients,
...
function CTForm({ clients, onClose, onSave }: {
  ...
  const [form, setForm] = useState({
    ...
    financialYear: "",
    deadline: "",
    status: "Pending" as CorporateTaxRecord["status"],
    assignedTo: "",
    taxAmount: "",
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="text-base font-semibold">Add Corporate Tax Record</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input placeholder="e.g. FY 2024-25" value={form.financialYear} onChange={e => setForm({ ...form, financialYear: e.target.value })}
                className="w-full px-3 py-2.5 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1.5">Tax Amount</label>
...
export default function CorporateTax() {
  ...
  const createRecord = useCreateCorporateTaxRecord({ mutation: { onSuccess: () => qc.invalidateQueries({ queryKey: getListCorporateTaxQueryKey() }) } });
              ...
              <tbody className="divide-y divide-border">
                {(records ?? []).map(r => (
                  <tr key={r.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3.5 font-medium">{r.taxAmount != null ? `£${Number(r.taxAmount).toLocaleString()}` : "—"}</td>
                    <td className="px-4 py-3.5 text-muted-foreground">{r.assignedTo}</td>
                  </tr>
                ))}
                {(!records || records.length === 0) && <tr><td colSpan={6} className="px-5 py-12 text-center text-muted-foreground">No corporate tax records found</td></tr>}
              </tbody>
            </table>
          )}
        </div>
